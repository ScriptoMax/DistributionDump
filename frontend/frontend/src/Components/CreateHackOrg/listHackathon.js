...
class ListHackathon extends Component {
  ...
  constructor(props) {
    ...
    this.finishDateChangeHandler = this.finishDateChangeHandler.bind(this);
  ...
  componentDidMount() {
    ...
    console.log(data);
    axios
      .get(`${url}/user/names/?screenName=${data.screenName}`)
      .then(response => {
  ...
  submitHackathon = e => {
    ...
    if (this.state.description.length < 10) {
      this.setState({
        error: true,
        errorMessage: "Description must be at least 10 alphanumeric characters"
      });
    } else if (this.state.startDate < date || this.state.finishDate < date) {
      this.setState({
        error: true,
        errorMessage: "Start and Finish Dates should be both greater than the current one"
      });
    } else if (this.state.startDate >= this.state.finishDate) {
      this.setState({
        error: true,
        errorMessage: "Finish date should be greater than start date"
      });
      ...
      axios
        .post(
          `${url}/hackathon/?name=${data.name}&description=${
            data.description
          }&startDate=${data.startDate}&finishDate=${
            data.finishDate
          }&ownerScreenName=${data.owner}&judgeScreenNames=${
            data.judge
          }&minTeamSize=${data.minTeam}&maxTeamSize=${data.maxTeam}&fee=${
            data.fee
          }&discount=${data.discount}&organisationNames=${data.sponsor}`
        )
        .then(response => {
            ...
            <div class="main-div-login">
              ...
              <div class="form-group">
                <label for="checkout" style={{ fontSize: "15px" }}>
                  Finish Date <span style={{ color: "red" }}>*</span>
                </label>
                <input
