...
class PaymentStatus extends Component {
  ...
  componentDidMount() {
    ...
    axios
      .get(`${url}/payment/report/${data.hackName2}`)
      .then(response => {
        console.log(
  ...
  render() {
    ...
    let teamInfo = this.state.teams.map(team => {
      members = team.teamMembers.map(m => {
        ...
        return (
          <div>
            <br />
            <span style={{ fontSize: "24px" }}>{m.teamMemberName}</span><br />
            <span style={{ color: "black", fontSize: "18px" }}>Payment Status {m.paymentStatus}
            <br />
            Amount {m.amount}
            <br />
            Payment Date {m.paymentDate}</span>
          </div>
        );
      });
      return (
          <div class="displaypropinfo container-fluid">
            <div class="col-sm-4">
              <div class="headline">
                <h3 class="hit-headline" style={{ marginTop: 0 }}>
                  <div
                      name="displayprop"
                  ...
                  <br />
                  <div name="displayprop" style={{ marginRight: "5px", padding: 0 }}>
                    {members}
                    <br />
                  </div>
                </h3>
    ...
    if (this.state.properties !== "") {
      return (
          <div>
            {redirectVar}
            <div className="main-div1" style={{backgroundColor: "#F7F7F8"}}>
              {navbar}
